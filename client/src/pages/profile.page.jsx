...
const ProfilePage = () => {
    ...
    const getBlogs = async ({ page = 1, user_id, create_new_arr = false }) => {
        const author = user_id || profile._id;
    ...
    useEffect(() => {
        ...
        // Only wipe the list when we are actually switching to a different profile.
        if (blogs !== null) {
            setBlogs(null);
        }
        ...
        axios
            ...
            .then(async ({ data: user }) => {
                ...
                await getBlogs({ page: 1, user_id: user._id, create_new_arr: true });
            })
            .catch((err) => {
                if (!cancelled) {
                    console.error(err);
                        ...
                        <>
                            {blogs === null ? (
                                ...
                                blogs.results.map((blog, i) => (
                                    <AnimationWrapper key={i} transition={{ duration: 0.4, delay: i * 0.06 }}>
